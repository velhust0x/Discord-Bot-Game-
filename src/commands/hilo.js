...
import { playHiLo } from '../lib/games.js';
import { EmbedBuilder } from 'discord.js';

export const name = 'hilo';

export async function execute(message, args) {
	const bet = Math.floor(Number(args[0]));
	if (!Number.isFinite(bet)) {
		const embed = new EmbedBuilder().setTitle('Usage').setDescription('!hilo <bet>').setTimestamp(Date.now());
